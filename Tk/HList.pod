...
#  license.tcl.
##
## Additional edits done by Rob Seegel
...
B<-activebackground>   B<-activeforeground>  B<-background> B<-borderwidth>
B<-cursor>  B<-disabledbackground> B<-disabledforeground>B<-exportselection>
B<-foreground>	       B<-font>	   B<-height>	B<-highlightcolor>
B<-highlightthickness> B<-relief>  B<-selectbackground>B<-selectforeground>
...
=item Name:	B<activeBorder>

=item Class:	B<ActiveBorder>

=item Switch:	B<-activeborder>

Specifies the type of border used when drawing the active element. The value can be any one of the following: B<dash> (a dotted line surrounding entry), B<solid> (a solid line surrounding entry), or B<none> (no border). The default value is B<dash>. 
...
Specifies a perl/Tk L<callback|Tk::callbacks> to be executed when the user browses through the entries in the HList widget. A "browse" is a motion that causes an entry to be selected, so this will be called when the Button is pressed while the mouse is stationary (for all modes) or while dragging (for extended and browse modes).
...
entry in the HList widget. Normally the user invokes a list entry by double-clicking it or pressing the Return key.
...
=item Name:	B<showActive>

=item Class:	B<ShowActive>

=item Switch:	B<-showactive>

Specifies whether or not to enable/disable highlighting for the 
active entry. This value can be a 1 or a 0. By default, the 
highlighting is set to B<1>.
...
The B<HList> method creates a new window (given by the $widget argument)
...
be displayed for this list entry. Please see L<Tk::DItem> for a list
of all display items. When a list entry is created by the B<itemCreate>, 
B<add> or B<addchild> widget methods, the type of its display item is 
determined by the B<-itemtype> option passed to these methods. If the
B<-itemtype> is omitted, then by default the type specified by this HList 
widget's B<-itemtype> option is used.

=head1 WIDGET METHODS

The B<HList> method creates a widget object. This object supports the 
B<configure> and B<cget> methods described in L<Tk::options> which can 
be used to enquire and modify the options described above. The widget 
also inherits all the methods provided by the generic L<Tk::Widget|Tk::Widget> 
class.
...
=item I<$hlist>-E<gt>B<activeSet>(I<$entryPath>)

Sets the active element to the one indicated by I<$entryPath>. The
active element is drawn with a dotted box around the element when
the HList has focus, and the -showactive option is set to 1. HList
will keep track of the active element internally regardless of whether
or not it is displayed.

=item I<$hlist>-E<gt>B<activeClear>

If an element is currently active, this will remove the active 
status, and remove the active highlighting if enabled.
...
The following configuration options can be given to configure the list entry:
...
Sets the anchor to the list entry identified by I<$entryPath>. 
The anchor is the end of the selection that is fixed while the
user is dragging out a selection with the mouse (in extended mode).

=item I<$hlist>-E<gt>B<anchorClear>

Removes the anchor, if any, from this HList widget. This has no
visible impact on the HList widget and does not affect current 
selection status.
...
column. See also L<"BUGS"> below. If an header display item 
already exists for this column, it will be replaced by the new 
item.  An optional parameter I<-itemtype> can be used to specify 
what type of display item should be created. If the I<-itemtype> 
is not given, then by default the type specified by this HList 
widget's B<-itemtype> option is used. Additional parameters, in 
I<option-value> pairs, can be passed to configure the appearance of 
the display item. Each I<option-value> pair must be a valid option 
for this type of display item or one of the following:
...
=item I<$hlist>-E<gt>B<info>(B<active>)

Returns the entryPath of the current active element, if any, of the
HList widget. If the anchor is not set, returns undef.

=item I<$hlist>-E<gt>B<info>(B<anchor>)

Returns the entryPath of the current anchor, if any, of the HList
widget. If the anchor is not set, returns undef.
...
=item I<$hlist>-E<gt>B<nearest>(I<y>, I<preciseEnd>?)
...
y-coordinate. The optional second parameter can set the method to
return undef if the y value is located below the bounding box
of the last entry.
...
pointer over the list entries, the entry beneath the mouse will be
selected and the B<-browsecmd> callback will be called with one parameter, 
the entryPath of the selected entry. Only one entry can be selected at a 
time. The B<-command> callback will be called when the user double-clicks 
on a list entry. 

=item [2]

If the B<-selectmode> is "single", only one entry can be selected at
a given time. When an entry is selected, the B<-browsecmd> callback will
be called with one parameter indicating the selected entry. A double
mouse click will cause the B<-command> callback to be called. 

=item [3]

If the B<-selectmode> is "multiple", the B<-browsecmd> callback 
will be called for single mouse click, and B<-command> callback
will be called for a double mouse click. A mouse click will
select or deselect a single entry. There can be more than one
non-adjacent or adjacent selected at the same time. The last
entry where the mouse button was released will be highlighted.
...
B<-browsecmd> callback will be called with one parameter - the entry 
directly beneath the mouse pointer. The B<-command> callback will be called
